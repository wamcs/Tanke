...
public class LocationProvinceAdapter extends BaseAdapter {
  ...
  public LocationProvinceAdapter(LocateUserActivityController controller, @Nullable String locateItem, @Nullable List<CityStruct> hotItems, @Nullable List<String> normalItems) {
    ...
    switch (getItemViewType(position)) {
      ...
      case VIEW_TYPE_HOT:
        ...
        for (int i = 0; i < hotItems.size(); i++) {
          ...
          layout.setBackgroundColor(parent.getContext().getResources().getColor(R.color.default_font_color));
        }
        break;
