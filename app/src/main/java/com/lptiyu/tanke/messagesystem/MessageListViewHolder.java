...
public class MessageListViewHolder extends BaseViewHolder<MessageNotificationList> {
    ...
    private WeakReference<MessageNotificationListDao> daoWeakReference;

    public MessageListViewHolder(ViewGroup parent, MessageNotificationListDao dao) {
    ...
    private void jumpToMessage(MessageNotificationList item, int type) {
        if (!item.getIsRead()) {
            mRedSpot.setVisibility(View.GONE);
